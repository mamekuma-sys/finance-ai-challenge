...
행동을 제시하고, 사건 상태에 맞는 전달 문구·행동 이벤트 원장·신고 준비 브리핑(서버 생성
...
|---|---|---|---|---|
| 주고객 | 송금·앱 설치·인증정보 노출 직후의 금융소비자 | AI 분석보다 공식 행동이 급함 | F-05 긴급 우회로, F-06 행동 카드·행동 이벤트 원장, F-07 고정 문구, F-08 브리핑 이중 구성 | AI 응답과 무관하게 첫 안전 행동을 보고 상태에 맞는 순서를 이해 |
...
- 사용자 자유 입력 사건은 서버 DB·공개 상황실·집계에 저장하지 않는다. 진행 상태·판정 결과·
  행동 이벤트 원장은 해당 브라우저의 `sessionStorage`에만 저장한다.
...
- `core`: **삭제 불가 코어 기능**이다. 하나라도 배포 검증에 실패하면 제출 자체가 차단되며,
  기능 제외나 범위 재협상으로 대체할 수 없다. 코어 E2E 게이트(§9.1)가 showcase 활성의
  전제 조건이다.
- `showcase`: 코어 E2E 게이트 통과 후에만 활성하는 시연·확장 기능이다. 배포 검증에
  실패하면 제출본에서 제외하고 관련 UI·문서를 제거한다. 활성 순서는 **1순위 F-16 REST·
  F-18 MCP**(agent 레이어 — 코어 게이트 통과 직후 최우선), **2순위 F-12 합성 공개 상황실·
  F-15 대시보드**다.
...
| F-06 | 행동 카드·행동 이벤트 원장 | 조치 결정 엔진(§4.5)으로 행동 카드를 구성하고 사실 수준별 이벤트를 `ActionFactEvent[]` 배열로 브라우저 세션에 기록한다. 경과 타이머는 사용자 확인 시점부터의 시간을 표시한다. | 상태 조합·사용자 확인 → 다음 행동·이벤트 이력·경과 시간 | `/` | core |
...
| F-13 | 개인 파이프라인 타임라인 | 사용자는 자기 브라우저에서만 6단계 실행 유형, 실제 단계 이벤트, 산출물 요약을 본다. SSE는 응답 중에만 유지하고 결과는 세션에 저장한다. | 현재 요청의 SSE → 로컬 단계 타임라인 | `/` | core |
...
| F-18 | 판정 MCP 도구 | F-16과 같은 판정 도구·스키마·쿼터·오류 정책을 MCP Streamable HTTP로 제공한다. 자유 텍스트는 REST와 동일한 목적 제한 토큰을 요구한다. 서버 사건 조회·쓰기는 제공하지 않는다. | `analyze_scam` 호출 → F-16과 동형의 결과 | `/mcp` | showcase |
| F-19 | 기계가독 안내 | `/llms.txt`와 개발자 문서에 목적, 판정 스키마, 데이터 정책, 쿼터, 한계, 안전성 리포트 위치를 공개한다(정적 파일이라 core 유지). | 정적 요청 → 평문 안내·개발자 문서 | `/llms.txt`, `/developers` | core |
| F-20 | 공개 안전성 리포트 | 비공개 홀드아웃 40건과 적대 입력 20건만으로 **예비 평가** 결과를 게시한다. §5.4의 판정 규칙 표·사전 임계값·분모·클래스·95% 구간·실패 범주·라벨링·튜닝 비노출을 명시한다. | 버전이 고정된 검증 실행 → 지표·실패 사례·재현 정보 | `/safety` | core |
...
F-20은 15-revision-plan-r2 E2의 티어 목록에 명시되지 않았으나, 패널 합의문의 코어 구현
순서 마지막 단계가 “독립 평가”이고 E4가 안전성 리포트 계약을 배포 차단 게이트로 강화했으
므로 core로 지정한다.
...
이벤트 원장, 단계 타임라인을 복구한다. 다른 브라우저·기기에서는 사용자 사건을 조회할 수
...
| 50초 | 행동 이벤트 원장에 사용자 확인값 기록 | `ui_event`/`user_statement` 출처와 이벤트 이력(시각·이전 상태) 구분 | F-06 |
...
- `scenario_id` 호출은 공개 가능하다. **자유 텍스트는 목적 제한 토큰을 요구한다.**
  ...
  **목적 제한 토큰 계약:**
  ...
  - UI는 기기 내 가림 확인 시 토큰을 자동 발급받아 사용하므로 사용자 계정·로그인은 필요
    없다(무로그인 유지). MCP 자유 텍스트 호출(F-18)도 동일 토큰이 필수다.
...
// 행동 이벤트 원장 — 브라우저 sessionStorage 전용, 서버 미전송
interface ActionFactEvent {
  event_id: string;
  action_id: string;
  occurred_at: string;            // ISO 8601, 브라우저 기록 시각
...
interface ActionCard {
  ...
  required_followup: string[];    // 전역 규칙 포함 (§4.5.1 ④)
...
interface AnalyzeResponse {
  ...
  actions: ActionCard[];          // 최대 4개, 초과분은 "다음 단계"로 접힘
  disclaimer: string;
}
```
...
| `/mcp` | POST | MCP Streamable HTTP의 `analyze_scam` | F-16과 동형의 결과 | 표준 MCP 오류, `401`, `429`, `503` |

### 4.3 MCP 도구와 `llms.txt`

MCP v1은 `analyze_scam` 도구 하나만 제공한다. 입력·출력·마스킹·목적 제한·쿼터·오류 정책은
F-16과 동일하다. **자유 텍스트 입력은 REST와 동일한 목적 제한 토큰(§4.1)이 필수이며**,
도구 description에 “호출 클라이언트가 기기 내에서 민감정보를 제거할 책임이 있다”를
명시한다. 서버 2차 마스킹은 MCP 경로에도 동일하게 적용한다. 사건 조회, 행동 사실 변경,
PDF 보관, 공식기관 처리 호출 도구는 제공하지 않는다.
...
| 필드 | 허용값 | 질문 의미 | 미확인 처리 |
|---|---|---|---|
| `transfer_state` | `not_sent / already_sent / unknown` | 돈을 보냈는가 | `unknown`이면 송금 여부 확인 질문을 최우선 삽입 |
...
`unknown`은 항상 별도 분기다. 모든 행은 판정 등급과 무관하게 적용한다.

#### 4.5.1 실행 알고리즘

1. **전체 수집:** 조건이 참인 **모든** 규칙 행을 수집한다(첫 일치 선택이 아님).
2. **위해 정렬:** 수집한 행을 `severity_rank`(§4.5.2)로 정렬한다. 핵심 4개 축
   (`transfer_state`, `device_compromise_state`, `credential_exposure_state`,
   `personal_data_exposure_state`) 중 `unknown`이 있으면 해당 축의 확인 질문 카드를
   **1번 카드로 최우선 삽입**한다.
3. **병합:** `merge_key`(채널×목적) 단위로 중복을 제거한다. 같은 `merge_key`의 행동은
   하나의 카드로 병합하며, 카드 문구·전제(`prerequisite`)는 더 높은 severity 행을 따르고
   목적 슬롯과 템플릿 버전은 병합된 행 전체의 합집합으로 나열한다.
4. **전역 필수 후속:** 필수 후속 절차는 규칙과 독립인 전역 규칙이다 —
   `transfer_state=already_sent`이면 `TPL-WRITTEN-FOLLOWUP-001`(전화 신청 시 3영업일
   이내 서면 신청)을 **어떤 병합 결과에서도** `required_followup`에 포함한다. 병합·중복
   제거·카드 수 제한으로 탈락할 수 없다.
5. **금지 행동 합집합:** `do_not_show_when`·금지 행동은 일치한 모든 행의 합집합이다.
6. **재번호·상한:** 우선순위를 1부터 재번호한다. 카드는 최대 4개이며 초과분은
   “다음 단계” 접힘 목록으로 내린다(전역 필수 후속은 접힘 대상에서 제외).
...
이때 확인 질문 카드는 §4.5.1 ②에 따라 1번 카드가 된다.

**`user_role=family_proxy` 수정자:** 모든 카드에 “가족을 대신해 확인 중”을 표시하고,
ECRM 온라인 신고 등 본인 제한 절차는 본인이 수행해야 함을 고정 문구
(`TPL-PROXY-SCOPE-001@1.0`)로 안내한다. 대리 실행이 가능하다는 표현은 금지한다.

#### 4.5.4 복합 조합 인수 스냅샷 5종

아래 스냅샷은 예시가 아니라 **인수 기준**이다(§9.3). 상태 필드는 명시하지 않은 축을
`none`(또는 `safe_device_available=yes`, `user_role=self`)으로 둔다.

| # | 상태 조합 | 기대 카드 순서 | 금지 행동(합집합) | 필수 후속 | 템플릿 버전 |
|---|---|---|---|---|---|
| a | `transfer=already_sent`, `device=suspected_app`, `safe_device=no` | 1 의심 기기 중지·안전 기기 확보 → 2 안전 기기에서 금융회사 대표번호·지급정지 요청(R1+R3 병합, 전제=별도 기기) → 3 안전 기기에서 112 신고·지급정지 연계 → 4 3영업일 서면 신청 후속 | 감염 의심 기기 금융 앱·대표번호 검색·인증정보 재입력, 재판정 대기, 일괄지급정지 대체 표시 | `TPL-WRITTEN-FOLLOWUP-001` 포함 | `TPL-SAFE-DEVICE-001@1.0`, `TPL-BANK-STOP-001@1.0`, `TPL-WRITTEN-FOLLOWUP-001@1.0` |
| b | `transfer=already_sent`, `device=remote_control`, `credential=shared`, `safe_device=yes` | 1 안전 기기에서 금융회사 대표번호·지급정지+인증정보 노출 통지(R2+R3+R4 병합) → 2 안전 기기에서 112 → 3 인증수단 폐기·재발급+악성 앱 검사(R2+R4 병합) → 4 3영업일 서면 신청 후속 | 감염 의심 기기 이용 긴급 조치, 재판정 대기, 일괄지급정지 대체 표시, AI 분석 대기, 상대 제공 번호·링크·앱 사용 | `TPL-WRITTEN-FOLLOWUP-001` 포함 | `TPL-SAFE-DEVICE-001@1.0`, `TPL-BANK-STOP-001@1.0`, `TPL-CREDENTIAL-RECOVERY-001@1.0`, `TPL-WRITTEN-FOLLOWUP-001@1.0` |
| c | `transfer=unknown`, `credential=shared` | 1 송금 여부 확인 질문(unknown 축 최우선) → 2 금융회사 대표번호·인증정보 노출 통지 → 3 112 상담 → 4 인증수단 폐기·재발급. 답변으로 `already_sent` 갱신 시 지급정지 목적과 전역 후속을 즉시 삽입해 재구성 | AI 분석 대기, 상대 제공 번호·링크·앱 사용, `낮음` 확정 표시 | `already_sent` 갱신 시 `TPL-WRITTEN-FOLLOWUP-001` 즉시 포함 | `TPL-UNDETERMINED-001@1.0`, `TPL-CREDENTIAL-RECOVERY-001@1.0` |
| d | `user_role=family_proxy`, `transfer=already_sent` | 1 금융회사 대표번호·지급정지 요청(“가족을 대신해 확인 중” 표시) → 2 112 신고 → 3 3영업일 서면 신청 후속 → 4 본인 제한 절차 안내(ECRM 온라인 신고 등은 본인 수행, 가족은 준비 보조) | 대리 신고·대리 접수 가능 표현, 재판정 대기, 일괄지급정지 대체 표시 | `TPL-WRITTEN-FOLLOWUP-001` 포함 | `TPL-BANK-STOP-001@1.0`, `TPL-WRITTEN-FOLLOWUP-001@1.0`, `TPL-PROXY-SCOPE-001@1.0` |
| e | `personal_data=shared`, `device=unknown`, `safe_device=unknown` | 1 기기 상태 확인 질문(앱 설치·원격제어 여부, unknown 축 최우선) → 2 추가 접촉·정보 제공 중단 → 3 공식 대표채널 교차 확인 → 4 1332 상담 경로 | 지급정지·신고 접수 확정 표현, `낮음` 안전 보증, (기기 상태 확인 전) 해당 기기 금융 앱 사용 권유 | — | `TPL-UNDETERMINED-001@1.0`, `TPL-OFFICIAL-VERIFY-001@1.0` |

### 4.6 행동 이벤트 원장

행동 사실은 단일 현재값이 아니라 **`ActionFactEvent[]` 이벤트 배열**로 기록한다. 각
이벤트는 `{event_id, action_id, occurred_at, state, source, previous_state}`를 가지며,
기존 이벤트를 덮어쓰지 않고 항상 새 이벤트를 추가한다(append-only). 정정도 새 이벤트로
기록한다. 저장은 브라우저 `sessionStorage` 전용이며 서버로 전송하지 않는다.
...
| 저장 영역 | 엔티티·필드 | 저장 원칙 |
|---|---|---|
| 브라우저 `sessionStorage` | `IncidentState`, 판정, 질문 답변 범주, `ActionFactEvent[]` 이벤트 원장, 파이프라인 이벤트, 템플릿 버전 | 사용자 사건 전용. 탭 세션 동안만 유지하며 서버 동기화 없음. 원문·직접 식별자·활성 URL·개인 부속면 필드는 저장하지 않음 |
...
한 출력에 여러 위반이 있으면 **사건 단위 최대 1회**로 세고 위반 유형별로 부기한다.
모든 비율에는 **Wilson score 95% 신뢰구간**을 병기한다.
...
7. 판정, 상태, 행동 카드, 행동 이벤트 원장의 모순 검사
...
| NF-08 | 남용 통제 | 목적 제한 토큰(발급·TTL 10분·scope·해시 보관·폐기 API), 토큰·IP·패턴 쿼터, 승인 CORS, 요청 본문 비로그, 입력 비반사 오류, 분산 호출·비용 급증·적대 반복 탐지를 적용한다. MCP 자유 텍스트에도 동일 토큰을 요구한다. |
...
|---|---|---|---|
| 실계좌·실차단 연동 | 계좌 조회, 은행 API 호출, 지급정지 실행·기관 결과 확인 | 공식 채널 순서와 고정 요청 문구, 사용자 진술 기반 이벤트 원장 | 금융회사 협약·보안·법률·책임 검토 |
...
1. **core 게이트:** §2.2의 모든 `core` 기능을 배포 URL에서 성공·빈 상태·오류 상태로
   검증한다. core 실패는 제출 차단 사유이며 기능 제외로 대체할 수 없다. `showcase`는
   core E2E 게이트 통과 후 활성하고(1순위 F-16·F-18, 2순위 F-12·F-15), 실패 시 제출본과
   UI에서 제거한다. `nice`는 통과한 경우에만 남긴다.
...
3. **결정 엔진 총함수:** §4.4의 6개 필드·열거형이 UI, REST, MCP, PDF에서 일치하고,
   §4.5.1 알고리즘이 모든 상태 조합에서 카드 1~4개를 산출하는지 속성 기반 테스트로
   검증한다. §4.5.4 인수 스냅샷 5종의 카드 순서·금지 행동·필수 후속·템플릿 버전이
   정확히 일치해야 하며, `transfer_state=already_sent`인 모든 조합에서
   `TPL-WRITTEN-FOLLOWUP-001` 포함을 전수 확인한다.
4. **이벤트 원장:** §4.6의 상태·출처 열거형과 `ActionFactEvent[]` append-only 구조,
   허용 전이 검증, 정정 이벤트를 확인한다. 자동 지표는
   `official_action_presented`·`dialer_opened`까지만 계산한다.
...
8. **평가 계약:** 안전성 리포트에 “예비 평가” 명명, §5.4.1 판정 규칙 표, §5.4.2 사전
   임계값의 버전 관리 등록 증적(평가 실행 전 커밋), 분모, 클래스별 건수, Wilson 95% 구간,
   실패 범주, 라벨링 절차(독립 라벨 또는 대체 절차 명시), 모델·프롬프트·규칙·템플릿 버전,
   프롬프트 튜닝 비노출을 포함한다.
...
11. **토큰·스키마 계약:** `POST /api/v1/tokens` 발급(TTL 10분, scope, 해시만 보관),
    `DELETE` 폐기, `Authorization: Bearer` 검증, `text` 존재 시 `masking_confirmed=true`
    조건부 필수(위반 `422`), MCP 자유 텍스트의 동일 토큰 요구, 쿼터, CORS, 입력 비반사
    오류, 남용 탐지를 UI 우회 REST와 MCP에서 각각 검증한다.
