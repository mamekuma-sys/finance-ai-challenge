...
- 기준 문서: [최종 주제 확정](../research/06-decision.md),
  ...
  [개정 계약 r3](./17-revision-plan-r3.md), [수법 DB 공개 출처](./05-method-db-sources.md),
  [문서 구조](./00-document-structure.md)
...
3. **병합:** `merge_key`(채널×목적) 단위로 중복을 제거한다. 같은 `merge_key`의 행동은
   ...
   템플릿 버전도 합집합으로 나열한다.
4. **전역 필수 후속:** `transfer_state=already_sent`이면 서면 신청 후속을 **독립 카드**
   (merge_key `procedure:written_followup`, `TPL-WRITTEN-FOLLOWUP-001`)로 생성한다.
   이 카드는 **항상 노출 카드 4개 안에 포함**되며 접힘·병합·상한으로 탈락할 수 없다.
5. **수정자 카드·금지 합집합:**
...
6. **재번호·상한·보존:** 우선순위를 1부터 재번호한다. 노출 카드는 최대 4개이며 초과분은
   **`next_steps` 필드에 전부 보존**해 “다음 단계” 접힘 목록으로 렌더링한다(소실 금지).
...
알고리즘·규칙·수정자에서 **기계적으로 도출**된다(도출 근거를 각 행에 병기). 상태 필드는
명시하지 않은 축을 `none`(또는 `safe_device_available=yes`, `user_role=self`)으로 둔다.
...
| KPI | 통과선 초기값 | 판정 방법 |
|---|---|---|
| ① 첫 안전 행동 노출 | Rule 0 p95 ≤ 2초 (NF-06 연결) | 배포 환경 측정 이벤트 |
...
| ⑤ AI 기여 | 사실 추출 누락률 상대 20% 이상 감소 시 우위 인정 (N≥20 사건) | 사전 정답 슬롯 자동 채점, 이해도는 5점 척도 참가자 평가(N≥5). 미달 항목은 결과 그대로 공개 + 규칙형 대체 검토 |
...
3. **결정 엔진 총함수:** §4.4의 6개 필드·열거형이 UI, REST, MCP, PDF에서 일치하고,
   ...
   ② `transfer_state=already_sent`인 모든 조합에서 서면 후속 독립 카드가 노출 4개 안에
   포함 ③ 접힘 행동의 `next_steps` 무손실 ④ 병합 카드의 `purpose_slots` 합집합 보존.
