...
3. 시행일을 확인할 수 없는 안내성 템플릿은 지시대로 확인일 `2026-07-25`를 날짜 필드에
   두고 `source_effective_date_confirmed=false`로 명시했다. 지급정지 템플릿과 서면 제출
   템플릿만 확인된 법 시행일 `2011-09-30`과 `true`를 사용했다. 이후 r5/r6 정정에 따라
   서면 제출 문구와 출처를 현행 계약에 맞추고 현재 버전을
   `TPL-WRITTEN-FOLLOWUP-001@1.1`로 갱신했다.
...
2026-07-25 정적 목업과 이 보고서의 후속 검수에서 다음 정정을 반영했다.

- r5 H1: 긴급하거나 부득이한 사유로 전화 또는 구술로 피해구제를 신청한 경우, 신청한
  날부터 3일 이내에 피해구제신청서를 해당 금융회사에 제출해야 한다는 현행 문구로
  통일했다.
- r5 H3~H5: 원문이 확인되지 않은 설문·피해 규모·시행일 수치와 측정 근거가 없는
  소요시간·응답시간·정확도·완주율 표기를 목업과 이 보고서의 검사 범위에서 차단했다.
- r6 I1: 서면 제출 카드에 현행 시행령 제3조와 경찰청·대한민국 정책브리핑의 1394 안내를
  보강했다. 서면 제출을 본 행동으로, 1394 피해상담·관계기관 연계를 후속 상담으로 표시한다.
- r6 I3: F-01 구현을 암시하지 않도록 세 목업의 상단 배지를 `구조화 상태 선택 데모`로
  바꾸고, 비교 목업의 미실측 데모 소요시간 표기를 제거했다.
- 서면 제출 템플릿의 현재 버전은 `TPL-WRITTEN-FOLLOWUP-001@1.1`이다. 다른 템플릿
  버전은 변경하지 않았다.

독립 검사는 기존 `app/src/**` 범위에 목업 HTML·목업 README·이 보고서를 더한다. 금지
목록 자체를 설명하는 검증·리뷰·개정 계약 문서는 명시적으로 제외한다.

```bash
cd /Users/paran/finance-ai-challenge
node app/scripts/check-forbidden-strings.mjs
```
