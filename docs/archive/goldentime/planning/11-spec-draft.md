# 골든타임(GoldenTime) 기능명세서 초안 — ⚠️ 폐기

> **이 문서는 2026-07-27 폐기됐다.** 컨셉이 「기록으로」 — 내 편의 사고조사실로 교체됐다.
> 현행 기능명세서: **`41-spec-draft.md`** / 재사용 대상은 거기 §10에 명시했다.
> 이 문서는 M0 계약 동결(개정 r1~r6·패널 5라운드)의 기록으로만 보존한다.
> **수치·기능·계약을 여기서 인용하지 말 것** — 골든타임 도메인 전용이다.
...
- **배포 URL(심사용, 무로그인):** https://app-three-phi-xcbdkemvcf.vercel.app
  (기계가독 안내 `/llms.txt`). Vercel 배포 전용 URL은 배포 보호가 걸려 외부 접근이
  불가하므로 **제출·검증에는 위 production 별칭만 사용한다.**
- **배포 환경 실측(2026-07-25, NF-06):** `Rule 0` 렌더 p95 **27.9ms**(n=12, median 20.6,
  최근접 순위 산식, 상한 2,000ms 충족). 측정 시작=긴급 응답 선택 입력 확정,
  종료=안전 카드 첫 페인트(double rAF). 같은 실행에서 가로 오버플로 0
  (360/390/768/1280/1440 × 라이트·다크), 상호작용 중 네트워크 요청 0건, 콘솔 에러 0건.
  **상태 전환 비교 재구성 p95와 연속 7일·N≥100 표본은 후속 실측 대상이다.**
