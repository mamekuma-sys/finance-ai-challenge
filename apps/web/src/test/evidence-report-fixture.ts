...
/**
 * 데모 endpoint가 돌려주는 것과 같은 모양의 합성 report.
 *
 * services/backend/src/rwa_guard/fixtures.py의 build_demo_report()와 맞춘다.
 * 계약이 바뀌면 여기와 backend fixture를 같은 변경에서 갱신한다.
 */
export const DEMO_REPORT = {
  ...
  lineage: { document: { document_id: "doc_synthetic_issuance_01" } },
  generated_at: "2026-08-25T03:00:10Z",
  is_synthetic: true,
} as unknown as EvidenceReport;

/**
 * 실제 Response를 돌려준다. 손으로 만든 stub은 openapi-fetch가 부르는
 * response.text() 같은 표면을 빠뜨려 실패 원인을 흐린다.
 */
export function stubEvidenceReportFetch(body: unknown = DEMO_REPORT, status = 200) {
  vi.stubGlobal(
    "fetch",
